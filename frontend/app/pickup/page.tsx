				...
				<div className="grid grid-cols-1 md:grid-cols-2 gap-5 items-start">
					...
					<div>
						<div className="flex items-center gap-2 mb-4">
							...
							<h2 className="text-xl font-bold text-[#1A1A2E]">{t.pickupTitle}</h2>
						</div>
						<div className="space-y-4">
							{pickups.map((pickup) =>
							(
								<div key={pickup.id} className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
									<div className="flex justify-between items-start mb-4">
										<FontAwesomeIcon icon={faTruck} className="text-[#B0B8C8] text-xl mt-1" />
										<div className={lang === "ar" ? "text-right" : "text-left"}>
											<p className="text-[#4F46E5] font-bold text-lg">{pickup.area}</p>
											<p className="text-[#B0B8C8] text-xs">#{pickup.id}</p>
										</div>
									</div>
									<div className="grid grid-cols-2 gap-4 mb-4">
										<div className={lang === "ar" ? "text-right" : "text-left"}>
											<p className="text-xs text-[#B0B8C8] mb-1">{t.sellerPhone}</p>
											<p className="font-bold text-[16px]">{pickup.seller_phone}</p>
										</div>
									</div>
									{pickup.picker && (
										<div className="flex items-center gap-2 mb-4">
									...
									<button
										...
										className="w-full border border-gray-200 rounded-xl py-2.5 text-[#E24B4A] font-semibold text-[16px] cursor-pointer hover:bg-red-50 transition-colors">
										{t.cancel}
									</button>
								</div>
							))}
					...
					<div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
						<div className="flex justify-between items-center mb-5">
							<h2 className="text-lg font-bold text-[#1A1A2E]">{t.newPickup}</h2>
						</div>
						<div className="space-y-4">

							{/* City - Combobox with Search */}
							<div ref={cityRef}>
								<label className={`block text-xs font-semibold text-[#505F76] mb-1 ${lang === "ar" ? "text-right" : "text-left"}`}>{t.city}</label>
								<div className="relative">
									<div className={`flex items-center gap-2 border rounded-xl px-4 py-3 transition-colors ${invalid("city") ? "border-red-400 bg-red-50" : "border-gray-200 focus-within:border-[#4F46E5]"}`}>
										<FontAwesomeIcon icon={faTruck} className="text-[#B0B8C8] text-[16px]" />
										<input
											value={city_search}
											onChange={(e) => {
											...
											className={`flex-1 outline-none text-[16px] ${lang === "ar" ? "text-right" : "text-left"} bg-transparent`}
											placeholder={t.cityPH}/>
										{city_search && (
											<button
												type="button"
												onClick={clearCitySearch}
												className="text-[#B0B8C8] hover:text-[#E24B4A] transition">
												<FontAwesomeIcon icon={faTimes} className="text-[16px]" />
											</button>
										)}
										<FontAwesomeIcon 
											icon={faChevronDown} 
											className={`text-[#B0B8C8] text-[16px] transition-transform ${isCityOpen ? "rotate-180" : ""}`} />
									</div>
										...
										<div className="absolute z-50 w-full bg-white border border-gray-200 rounded-xl shadow-lg mt-1 max-h-48 overflow-y-auto">
											{filteredNetworks.map((n: any) => (
												<button
													...
													className={`w-full text-start px-4 py-2.5 text-[16px] hover:bg-[#F0F2FF] transition-colors ${network === n.id ? "bg-[#F0F2FF] text-[#4F46E5]" : ""}`}
												>
													{n.name}
												</button>
											))}
									...
									{isCityOpen && city_search && filteredNetworks.length === 0 && (
										<div className="absolute z-50 w-full bg-white border border-gray-200 rounded-xl shadow-lg mt-1 p-4 text-center text-[16px] text-gray-400">
											{t.noCitiesFound}
										</div>
									)}
								</div>
							...
							<div ref={regionRef}>
								<label className={`block text-xs font-semibold text-[#505F76] mb-1 ${lang === "ar" ? "text-right" : "text-left"}`}>{t.region}</label>
								<div className="relative">
									<div className={`flex items-center gap-2 border rounded-xl px-4 py-3 transition-colors ${!network ? "opacity-50" : ""} ${invalid("region") ? "border-red-400 bg-red-50" : "border-gray-200 focus-within:border-[#4F46E5]"}`}>
										<FontAwesomeIcon icon={faMapMarked} className="text-[#B0B8C8] text-[16px]" />
										<input
											value={region_search}
											onChange={(e) => {
										...
										<FontAwesomeIcon 
											icon={faChevronDown} 
											className={`text-[#B0B8C8] text-[16px] transition-transform ${isRegionOpen ? "rotate-180" : ""}`} 
										/>
									</div>
										...
										<div className="absolute z-50 w-full bg-white border border-gray-200 rounded-xl shadow-lg mt-1 max-h-48 overflow-y-auto">
											{filteredAreas.map((a: any) => (
												<button
													...
													className="w-full text-start px-4 py-2.5 text-[16px] hover:bg-[#F0F2FF] transition-colors">
													{a.name}
												</button>
											))}
										</div>
									...
									{network && areas.length === 0 && !isRegionOpen && (
										<p className="text-xs text-gray-400 mt-1">{t.noAreasAvailable}</p>
									)}
								</div>
							...
							{/* Phone */}
							<div>
								<label className={`block text-xs font-semibold text-[#505F76] mb-1 ${lang === "ar" ? "text-right" : "text-left"}`}>{t.phone}</label>
								<div className={`flex items-center gap-2 border rounded-xl px-4 py-3 transition-colors ${invalid("phone") ? "border-red-400 bg-red-50" : "border-gray-200 focus-within:border-[#4F46E5]"}`}>
									<FontAwesomeIcon icon={faPhone} className="text-[#B0B8C8] text-[16px]" />
									<input
										value={form.phone}
							...
							<button
								...
								className="w-full bg-[#4F46E5] text-white font-semibold text-[16px] py-3 rounded-xl cursor-pointer hover:bg-[#4338CA] transition-all duration-300 mt-2"
							>
								{t.submit}
							</button>
